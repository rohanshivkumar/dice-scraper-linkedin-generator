...
def parse(keyword, place):
    ...
    try:
        ...
        while(i < 3):
            ...
            company_suffix = '//span[@class="hidden-xs"]//span[@class="compName"]/text()'
            name_suffix = '//a/span/text()'
            job_url_suffix = '//a[@href]'
            loc_suffix = '//span[@class="jobLoc"]/text()'
            i += 1
            next_url = base_search_url + "&p=%s"%(i+1)
            print(next_url)
            ...
            for job in listings:
                ...
                job_url =  browser.find_element_by_xpath(XPATH_JOB_URL).get_attribute("href")
            
                company_name = browser.find_element_by_xpath(XPATH_COMPANY).text
                job_key = job_name + "-" + company_name + "-" + job_location
                if(job_key in job_listings):
                    continue
                search_term = company_name +" " + job_name + ' \"manager\" OR \"director\" linkedIn' 
